...
Arduino_ESP32RGBPanel* rgbpanel =
    new Arduino_ESP32RGBPanel(40 /* DE */, 41 /* VSYNC */, 39 /* HSYNC */, 42 /* PCLK */, 45 /* R0 */, 48 /* R1 */, 47 /* R2 */, 21 /* R3 */, 14 /* R4 */, 5 /* G0 */, 6 /* G1 */,
...
void setup() {
    ...
    if (!disp_draw_buf) {
        ...
        xTaskCreatePinnedToCore(displayStatusMessages_t, "DisplayStatus", 8192, NULL, 0, NULL, 0);
    }
}
...
void displayStatusMessages_t(void* pvParameters) {
    ...
    while (true) {
        if (xQueueReceive(statusMessageQueue, &receivedMsg, portMAX_DELAY) == pdTRUE) {
            strncpy(statusMessageValue, receivedMsg.text, CHAR_LEN);
            ...
            strncpy(statusMessageValue, "", CHAR_LEN);
        }
    }
}
...
void logAndPublish(const char* format, ...) {
    ...
    xQueueSend(statusMessageQueue, &msg,
               0); // Use 0 for no-wait if queue is full
}
