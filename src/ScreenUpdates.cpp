  ...
  if(solar.currentUpdateTime > 0)
    {
      ...
      if(solar.batteryPower > 0.1)
        {
          ...
          if((floor(remain_hours) == 1) && (remain_minutes > 0))
            {
              snprintf(tempString, CHAR_LEN,
                       "%2.0f hour %i mins\n remaining\n Until %s",
                       remain_hours, remain_minutes_round % 60, time_buf_end);
            }
          else
            {
              if((remain_minutes > 0)
                 && (remain_hours < MAX_SOLAR_TIME_STATUS))
                {
                  snprintf(tempString, CHAR_LEN,
                           "%2.0f hours %i mins\n remaining\n Until %s",
          ...
          lv_obj_set_style_arc_color(ui_BatteryArc, lv_color_hex(COLOR_RED),
                                     LV_PART_INDICATOR
                                       | LV_STATE_DEFAULT); // Set arc to red
      ...
      else
        {
          ...
          if(solar.batteryPower < -0.1)
            {
              snprintf(tempString, CHAR_LEN, "Charging %2.1fkW",
                       -solar.batteryPower);
              lv_label_set_text(ui_ChargingLabel, tempString);
              ...
              lv_obj_set_style_arc_color(
                ui_BatteryArc, lv_color_hex(COLOR_GREEN),
          ...
          else
            {
              ...
              lv_obj_set_style_arc_color(
                ui_BatteryArc, lv_color_hex(0x2095F6),
          ...
          lv_obj_set_style_arc_color(ui_SolarArc, lv_color_hex(COLOR_GREEN),
                                     LV_PART_INDICATOR
                                       | LV_STATE_DEFAULT); // Set arc to green
      ...
      snprintf(tempString, CHAR_LEN, "Min %2.0f\nMax %2.0f",
               solar.today_battery_min, solar.today_battery_max);
      ...
      if(solar.today_buy != 0.0 || solar.month_buy != 0.0)
        {
          char *boughtToday = format_integer_with_commas(
            (int)floor(solar.today_buy * ELECTRICITY_PRICE));
          char *boughtMonth = format_integer_with_commas(
            (int)floor(solar.month_buy * ELECTRICITY_PRICE));
...
char *format_integer_with_commas(long long num)
{
  ...
  char buffer[25]; // A buffer to hold the number string without commas. long
                   // long is at most 19 digits.
  ...
  char *result_str
    = (char *)malloc(total_len + 1); // +1 for the null terminator.
  if(!result_str)
    {
  ...
  for(int i = len - 1; i >= 0; i--)
    {
      ...
      // Add a comma if it's the end of a group of three digits and we're not
      // at the start.
