    ...
    if (solar.batteryPower > 0.1) {
      ...
      if ((floor(remain_hours) == 1) && (remain_minutes > 0)) {
        snprintf(tempString, CHAR_LEN,
                 "%2.0f hour %i mins\n remaining\n Until %s", remain_hours,
                 remain_minutes_round % 60, time_buf_end);
      } else {
        if ((remain_minutes > 0) && (remain_hours < MAX_SOLAR_TIME_STATUS)) {
          ...
          snprintf(tempString, CHAR_LEN, ""); // Don't print for too long time
        }
      }
...
char *format_integer_with_commas(long long num) {
  ...
  for (int i = len - 1; i >= 0; i--) {
    ...
    // Add a comma if it's the end of a group of three digits and we're not at
    // the start.
